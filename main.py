...
class Opas:
    ...
    def get_month_html(self) -> str:
        ...
        # 翌月の週ごとに HTML を取得する
        weekly_htmls = []
        week_count = self.get_week_count(self.first_week.month, self.first_week.day)
        for i in range(week_count):
            target = self.first_week + relativedelta(weeks=+i)
            logging.info(f'week{week_count}: {target}')
    ...
    def get_week_count(self, month, day):
        """HTMLを取得する週の数を取得する"""
        # 23日～月末までの場合は8週
        if day >= 23:
            return 8
...
if __name__ == '__main__':
    serve(app, host='0.0.0.0', port=int(os.environ.get("PORT", 8080)))  # FlaskアプリをWaitressで稼働させる
