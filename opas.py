...
class Opas:
    ...
    def get_month_html(self) -> str:
        ...
        for i in range(5):
            target = first_week + relativedelta(weeks=+i)
    ...
    def add_gym_shisetu(self, timeframe_count, tr, base_date, shisetu_name):
        if timeframe_count == 3:
            ...
            for m, a, n in zip(mornings, afternoons, nights):
                if "facmdstime" in m.get('class'):
                    ...
                    target_date = base_date + \
                        relativedelta(days=self.day_index)
                    date_str = target_date.strftime(DATE_FORMAT)
                    m_status = self.get_vacant_status(
                        m.find("img").get("src") + m.text)
                    self.gyms[self.gym_name][shisetu_name][TIME_MORNING][date_str] = m_status
                    a_status = self.get_vacant_status(
                        a.find("img").get("src") + a.text)
                    self.gyms[self.gym_name][shisetu_name][TIME_AFTERNOON2][date_str] = a_status
                    n_status = self.get_vacant_status(
                        n.find("img").get("src") + n.text)
            ...
            for m, a, e, n in zip(mornings, afternoons, evenings, nights):
                if "facmdstime" in m.get('class'):
                    ...
                    e_status = self.get_vacant_status(
                        e.find("img").get("src") + e.text)
    ...
    def add_gym_for_shisetu2(self, tr, base_date, shisetu1_name, shisetu2_name):
        # TODO shisetu_name は配列にしてループすれば１つの関数でいける
    ...
    def set_first_week(self, tr, shisetu, base_date, week_index):
        ...
        if len(shisetu) == 1:
            ...
            self.add_gym_shisetu(timeframe_count, tr, base_date, shisetu_name)
        else:
            ...
            self.add_gym_for_shisetu2(tr, base_date, shisetu1_name, shisetu2_name)
    ...
    def set_after_first_week(self, tr, shisetu, base_date, week_index):
        if len(shisetu) == 1:
            ...
            self.add_gym_shisetu(timeframe_count, tr, base_date, shisetu[0].text)
        else:
            self.add_gym_for_shisetu2(tr, base_date, shisetu[0].text, shisetu[1].text)
    ...
    def get_vacant(self, debug: int):
        ...
        if debug == 1:
            ...
            # with open(path) as f:
            #     html = f.read()
            # vacant_list = self.get_vacant_list(html)
            # with open('./output.json') as f:
            #     gym_json = f.read()
            # vacant_list = json.loads(gym_json)
            # message = self.create_message_from_list(vacant_list)
